...
@section("title", "MNS Game | Редактирование проекта")
    ...
    <div class="container max-w-full px-4 mx-auto text-left md:max-w-none md:text-center">
        <div class="lg:grid grid-cols-1 max-w-6xl mx-auto">
                        ...
                        <div class="tooltip-custom" data-tooltip="Рейтинг проекта">
                            <div class="rounded-3 px-2 py-1 text-orange-400 font-semibold">
                                <div class="text-center mr-1 items-center justify-center">
                                    ...
                                    <span class="inline ml-1 align-middle font-semibold text-sm text-gray-500 mt-[1%]">{{ $server->rating }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div id="secondColumn" class="my-4">
                <form class="w-full" enctype="multipart/form-data" action="{{ route("saveserver") }}" method="POST" id="form_input" onsubmit="inputFilters();">
                    @csrf
                    <div class="flex flex-wrap -mx-3 mb-6">
                        ...
                        <div class="lg:w-2/3 w-full px-3 lg:my-4">
                            <input value="{{ $server->title }}" name="server_title" class="@error('server_title') !border-red-500 @enderror appearance-none block w-full text-gray-700 border border-gray-200 rounded py-2 px-3 mb-3 mx-0 lg:mx-24 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 text-sm" id="server-title" type="text" placeholder="Какое будет название у вашего сервера?" required>
                            @error('server_title')
                    ...
                    <div class="flex flex-wrap -mx-3 mb-6">
                        ...
                        <div class="lg:w-2/3 w-full px-3 lg:my-4">
                            <textarea name="server_description" id="server-description" class="@error('server_description') !border-red-500 @enderror w-full h-16 px-3 py-2 text-sm text-gray-700 placeholder-gray-600 border rounded focus:shadow-outline" placeholder="Чтобы вы хотели рассказать о своём сервере игрокам?" required>{{ $server->description }}</textarea>
                    ...
                    <div class="flex flex-wrap -mx-3 mb-2">
                        ...
                        <div class="lg:w-2/3 w-full px-3 lg:my-4">
                            <select id="game-title" name="game_title" class="@error('game_title') !border-red-500 @enderror w-full h-10 px-3 text-sm placeholder-gray-600 border rounded appearance-none focus:shadow-outline" onchange="getFilters()">
                                <option value="{{ $server->game->title }}" selected>{{ $server->game->title }}</option>
                                @foreach($games as $game)
                                    @if($game->title != $server->game->title)
                    ...
                    <div class="flex flex-wrap -mx-3 mb-2" id="filters">
                        <div class='lg:w-1/3 w-full px-3 lg:my-2'>
                            <label for='game-filters' class='block text-md tracking-wide text-gray-700 font-bold mb-2 text-left'>Категории сервера</label>
                            <span class='block text-md tracking-wide text-gray-700 mb-2 text-left'>Этот параметр определяет, в какие категории попадет ваш сервер. Выбирайте только те категории, которые действительно присутствуют на вашем сервере. В противном случае, редактирование сервера будет отключено, а сам сервер понижен в рейтинге. <br><br><strong>Внимание!</strong> В случае полного игнорирования правил выбора параметров, сервер будет удален из мониторинга без возможности восстановления!</span>
                        </div>
                        <div class='lg:w-2/3 w-full px-3 lg:my-2'>
                            <div class='flex flex-col items-center relative'>
                                <div class='w-full'>
                                    <div class='p-1 flex border border-gray-200 bg-white rounded-t-lg'>
                                        <div class='flex flex-auto flex-wrap' id='filters-input'>
                                            @foreach($server->filters as $filter)
                                                <div class="flex justify-center items-center m-1 font-medium py-1 px-2 bg-white rounded-md text-indigo-700 bg-indigo-100 border border-indigo-300 h-[26px]" id="filter-id-{{ $filter->id }}-input">
                                                    <div class="text-xs font-normal leading-none max-w-full flex-initial">{{ $filter->filter }}</div>
                                                    <div class="flex flex-auto flex-row-reverse">
                                                        <div>
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x cursor-pointer hover:text-indigo-400 rounded-full w-4 h-4 ml-2" data-select-parent="filter-id-{{ $filter->id }}" onclick="removeFromSelect(this)">
                                    ...
                                    <div class='items-stretch w-full mb-4 z-20 absolute bg-gray-100 top-[42px] !rounded-b-lg hover:rounded-b-lg drop-shadow-lg filter-scroll filter-scroll-mnsgame'>
                    ...
                    <div class="flex flex-wrap -mx-3 mb-2">
                        ...
                        <div class="lg:w-2/3 w-full px-3 lg:my-4">
                            <input value="{{ $server->site }}" name="server_site" class="@error('server_site') !border-red-500 @enderror appearance-none block w-full text-gray-700 border border-gray-200 rounded py-2 px-3 mb-3 mx-0 lg:mx-24 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 text-sm" id="server-site" type="text" placeholder="Ссылка на сайт сервера">
                            @error('server_site')
                                <span class="flex items-center font-medium tracking-wide text-red-500 text-xs mt-1 ml-1">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="flex flex-wrap -mx-3 mb-2">
                        ...
                        <div class="lg:w-2/3 w-full px-3 lg:my-4">
                            <input value="{{ $server->vk }}" name="server_vk" class="@error('server_vk') !border-red-500 @enderror appearance-none block w-full text-gray-700 border border-gray-200 rounded py-2 px-3 mb-3 mx-0 lg:mx-24 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 text-sm" id="server-vk" type="text" placeholder="Ссылка на сообщество Вконтакте">
                            @error('server_vk')
                            <span class="flex items-center font-medium tracking-wide text-red-500 text-xs mt-1 ml-1">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="flex flex-wrap -mx-3 mb-2">
                        ...
                        <div class="lg:w-2/3 w-full px-3 lg:my-4">
                            <input value="{{ $server->discord }}" name="server_discord" class="@error('server_discord') !border-red-500 @enderror appearance-none block w-full text-gray-700 border border-gray-200 rounded py-2 px-3 mb-3 mx-0 lg:mx-24 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 text-sm" id="server-discord" type="text" placeholder="Ссылка-приглашение в Discord">
                    ...
                    <div class="flex flex-wrap -mx-3 mb-2">
                        ...
                        <div class="lg:w-2/3 w-full px-3 lg:my-4">
                            <input value="@if(!$server->is_launcher){{ $server->server_data }}@endif" name="server_ip" class="@error('server_ip') !border-red-500 @enderror @if($server->is_launcher) hidden @endif appearance-none block w-full text-gray-700 border border-gray-200 rounded py-2 px-3 mb-3 mx-0 lg:mx-24 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 text-sm" id="server-ip" type="text" placeholder="IP адрес сервера">
                            <input value="@if($server->is_launcher){{ $server->server_data }}@endif" name="launcher_link" class="@error('launcher-link') !border-red-500 @enderror @if(!$server->is_launcher) hidden @endif appearance-none block w-full text-gray-700 border border-gray-200 rounded py-2 px-3 mb-3 mx-0 lg:mx-24 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 text-sm" id="server-launcher" type="text" placeholder="Ссылка страницу скачивания лаунчера/клиента сервера">
                    ...
                    <div class="flex flex-wrap -mx-3 mb-2">
                        ...
                        <div class="lg:w-2/3 w-full px-3 lg:my-4">
                            <div class="flex">
                                <input value="{{ $server->callback }}" name="server_callback" class="@error('server_callback') !border-red-500 @enderror appearance-none block text-gray-700 border border-gray-200 rounded py-2 px-3 mb-3 mx-0 lg:mx-24 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 text-sm inline flex-auto" id="server-callback" type="text" placeholder="Ссылка на callback сайта">
                    ...
                    <div class="flex flex-wrap -mx-3 mb-2">
                        ...
                        <button type="submit" class="bg-indigo-500 hover:bg-indigo-700 text-white font-bold py-2 px-12 border border-blue-700 rounded mx-auto">Сохранить изменения</button>
                    </div>
                </form>
            </div>
        </div>
...
@section("scripts")
    ...
    <script>
        ...
        function selectFilter(element){
            let selection = element.firstElementChild;
        ...
        function removeFromSelect(element){
            let filter_suggestion = document.getElementById(element.getAttribute("data-select-parent")).firstElementChild;
